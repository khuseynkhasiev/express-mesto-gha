...
const createCard = async (req, res) => {
  ...
  try {
    ...
    if (e.name === 'ValidationError' || e.name === 'CastError') {
      console.log(e);
      return res.status(ERROR_INACCURATE_DATA).send({message: 'Переданы некорректные данные при создании карточки'});
    }
    console.error(e);
    return res.status(ERROR_INTERNAL_SERVER).send({message: 'На сервере произошла ошибка'});
  }
...
const putCardLike = async (req, res) => {
  ...
  try {
    ...
    if (e.name === 'ValidationError' || e.name === 'CastError') {
      console.log(e);
      return res.status(ERROR_INACCURATE_DATA).send({message: 'Переданы некорректные данные для постановки/снятии лайка'});
    }
    console.error(e);
    return res.status(ERROR_INTERNAL_SERVER).send({message: 'На сервере произошла ошибка'});
  }
